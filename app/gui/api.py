...
rf = 1.0
inf = 1e15
template_kwargs = {'template':"main", 'proxy':'', 'refresh_period': inf}
...
def naive() -> str:    
    ...
    if rq.method == 'POST' and state[0].value == 0:
        ...
        template_kwargs['refresh_period'] = rf
        ...
        if state[2].value != 2:
            output = f"<b> Status: {states[state[2].value]} </b> <br> Iteration : {state[0].value}/{state[1].value}"
        else:
            template_kwargs['refresh_period'] = inf
            output = f"<b> Status: {states[state[2].value]} </b> <br> Result : {state[3]['out']}"
